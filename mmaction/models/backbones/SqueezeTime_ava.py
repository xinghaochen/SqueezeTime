...
class BatchNorm3d(nn.BatchNorm3d):
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True,
        ...
        super(BatchNorm3d,self).__init__( num_features//time_len, eps, momentum, affine, track_running_stats)
    def forward(self, input: Tensor) -> Tensor:
        N,CT,H,W=input.shape
        input=input.view(N,-1,time_len,H,W)
        input= super().forward(input)
        input=input.view(N,-1,H,W)
        return input
class SpatialConv(nn.Module):
    def __init__(self,dim_in,dim_out,pos_dim=7):
        ...
        self.glo_conv=nn.Sequential(nn.Conv2d(dim_in,16,kernel_size=3,stride=1,padding=1,groups=1),nn.BatchNorm2d(16),nn.ReLU(inplace=True),nn.Conv2d(16,16,kernel_size=7,stride=1,padding=3),nn.BatchNorm2d(16),nn.ReLU(inplace=True),nn.Conv2d(16,dim_out,kernel_size=3,stride=1,padding=1,groups=1),nn.Sigmoid())
...
class Conv2d(nn.Module):
    def __init__(
        ...
        self.spatial_conv=SpatialConv(dim_in=in_channels,dim_out=out_channels,pos_dim=pos_dim)#nn.Conv2d(in_channels=in_channels,out_channels=int(out_channels*(1-ghost_ratio)),kernel_size=1,stride=1,padding=0,groups=1,bias=bias)
    def forward(self,x):
...
class ResNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 shortcut_type='B',
                 widen_factor=1.0,
                 n_classes=400,dropout=0.2,freeze_bn=False,spatial_stride=[1,2,2,2],pos_dim=[64,32,16,8]):
        super().__init__()
    ...
    def forward(self, x): 
        ...
        x = self.layer1(x)#56/64
        x = self.layer2(x)#28/32
        x = self.layer3(x)#14/16
        x = self.layer4(x)#7/8
...
class SqueezeTime_ava(nn.Module):
    def __init__(self,depth=50,widen_factor=1.0,dropout=0.5,input_channels=48,n_classes=400,load=None,freeze_bn=False,spatial_stride=[1,2,2,2],pos_dim=[64,32,16,8]):
        super(SqueezeTime_ava,self).__init__()
...
if __name__ == '__main__':
    ...
    model.eval()
    # flops, params = get_model_complexity_info(model, (3,16, 224, 224), as_strings=True, print_per_layer_stat=True)
    # print('Flops:  ' + flops)
    # print('Params: ' + params)
